...
class Generator
{
    ...
    public static function listFilterOnlyValue(
        ...
        foreach ($list as $list_item) {
            if ($list_item['id'] == $item[$column]) {
                $is_exist = true;
                $item[$column] = $list_item[$list_column] . ' [<strong>'
                    . $list_item['id'] . '</strong>]';
                break;
            }
        }
    ...
    public static function simpleFilter(&$item, $object, $column, $link, $name)
    {
        if (!empty($object)) {
            ...
            $item[$column]
                = "<strong class='text-danger'> $name not found </strong>";
        }
    }
    ...
    public static function genTableForeignLink($info, $value)
    {
        $table = $info['foreign_table'];
        $res_key = 'id';
        $res_name = $info['foreign_column'];
//        $key = $info['inner_column'];
        ...
        <div>
            [<?=$value?>]
            <a class="btn-link" href="list.php?cat=<?= $table ?>&get_column=<?= $res_key ?>&get_value=<?= $value ?>">
                <?= $result[$res_name] ?>
            </a>
        </div>
    ...
    public static function generateStandartTablePage($table)
    {
        ...
        <input type="hidden" class="table_columns"
               value='<?= Helper::json_encode($data_columns) ?>'>
